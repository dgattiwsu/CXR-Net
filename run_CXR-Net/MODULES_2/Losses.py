...
def cce_cluster_loss(y_true,y_pred):
    
    y_pred_max = tf.reduce_max(y_pred,axis=1,keepdims=True)
    y_true = tf.where(tf.equal(y_pred,y_pred_max),1.0,0.0)
    y_true = tf.cast(y_true, y_pred.dtype)
    
    return K.categorical_crossentropy(y_true, y_pred, from_logits=False)

def categorical_cluster_accuracy(y_true, y_pred):

    y_true_mod = tf.where(tf.greater(y_pred,0.5),1.0,0.0)    
    tf.dtypes.cast(y_true_mod, tf.float32)

    return tf.cast(tf.equal(tf.argmax(y_true_mod, axis=-1), tf.argmax(y_pred, axis=-1)),tf.float32)
...
def other_metrics_binary_class(y_true,y_pred,threshold):
    y_pred = (y_pred > threshold)*1
    TP = tf.reduce_sum(y_true * y_pred).numpy()
    FP = (tf.reduce_sum(y_pred) - TP).numpy()
    FN = (tf.reduce_sum(y_true) - TP).numpy()
